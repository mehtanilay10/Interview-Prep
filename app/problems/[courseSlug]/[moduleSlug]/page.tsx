...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  ...
  if (!mod) return {};
  return {
    title: `${mod.title} — Problems`,
    description: mod.description,
  };
}

export default async function ProblemsModuleDetailPage({ params }: Params) {
    ...
    <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6">
      ...
      <div className="mb-8">
        ...
        <div className="flex flex-wrap items-center gap-4 text-sm text-fg-muted">
          <span className="flex items-center gap-1">
            ...
            {pluralize(lessons.length, 'problem')}
          </span>
          <span className="flex items-center gap-1">
      ...
      <div className="mb-10">
        <h2 className="mb-4 text-xl font-bold text-fg-default">
          {lessons.length > 0 ? `${lessons.length} Problems` : 'Problems'}
        </h2>
        ...
        {lessons.length > 0 ? (
          ...
          <div className="rounded-xl border border-dashed border-border p-8 text-center">
            <p className="text-sm text-fg-muted">Problems coming soon for this module.</p>
          </div>
        )}
      </div>
      ...
      <div className="flex items-center justify-between gap-4 border-t border-border pt-8">
        {prevModule ? (
          <Link
            href={`/problems/${courseSlug}/${prevModule.slug}`}
        ...
        {nextModule && (
          <Link
            href={`/problems/${courseSlug}/${nextModule.slug}`}
            className="flex items-center gap-2 rounded-lg border border-border px-4 py-2.5 text-sm font-medium text-fg-muted transition-colors hover:border-accent-fg hover:text-accent-fg"
          >
