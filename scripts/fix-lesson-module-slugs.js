...
for (const dir of courseDirs) {
  ...
  for (const entry of entries) {
    ...
    for (const file of files) {
      ...
      if (lesson.moduleSlug !== entry) {
        console.log(`Fixing ${dir}/${entry}/${file}: ${lesson.moduleSlug} -> ${entry}`);
        lesson.moduleSlug = entry;
...
console.log(`Fixed ${fixed} lesson moduleSlugs to match directory names.`);
